...
from fire_rescue_rl.agents.expert_regularized_dqn import ExpertRegularizedDQN
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train expert-regularized DQN on multi-UGV rescue.")
    ...
    parser.add_argument("--expert-episodes", type=int, default=160)
...
def behavior_clone(model: ExpertRegularizedDQN, observations: np.ndarray, actions: np.ndarray, steps: int, seed: int) -> None:
    if steps <= 0:
        return
    obs_tensor = th.as_tensor(observations, dtype=th.float32, device=model.device)
    action_tensor = th.as_tensor(actions, dtype=th.long, device=model.device)
    ...
    for _ in range(steps):
        ...
        loss = F.cross_entropy(model.q_net(obs_tensor[indices]), action_tensor[indices])
    ...
    print(f"Initial BC done on {len(actions)} samples; final loss={losses[-1]:.4f}.")


def warm_start_replay(model: ExpertRegularizedDQN, env_config: dict, episodes: int, seed: int, expert_name: str) -> None:
    expert = build_expert(expert_name)
    added = 0
    for episode in range(episodes):
        env = FireRescueMultiUGVEnv.from_config(env_config)
        ...
        while not (terminated or truncated):
            ...
            obs = next_obs
            added += 1
    print(f"Warm-started replay with {added} expert transitions.")
...
def main() -> None:
    ...
    log_dir = ROOT / "outputs" / "logs" / "dqn_multi_er" / f"{name}_seed{args.seed}{expert_suffix}"
    ...
    if args.init_model:
        model = ExpertRegularizedDQN.load(args.init_model, env=env, seed=args.seed, tensorboard_log=str(log_dir))
        model.verbose = 1
        print(f"Loaded initial model: {args.init_model}")
    else:
        model = ExpertRegularizedDQN(env=env, seed=args.seed, verbose=1, tensorboard_log=str(log_dir), **model_kwargs)

    observations, actions = collect_expert_dataset(env_config, args.expert_episodes, args.seed, args.expert)
    if not args.init_model:
        behavior_clone(model, observations, actions, args.bc_steps, args.seed)
    model.set_expert_dataset(
        observations,
        actions,
        bc_coef=args.bc_coef,
        bc_batch_size=args.bc_batch_size,
    )
    warm_start_replay(model, env_config, max(1, args.expert_episodes // 2), args.seed, args.expert)
    ...
    final_path = model_dir / f"dqn_er_{name}_seed{args.seed}{expert_suffix}.zip"
    model.save(final_path)
    best_path = model_dir / "best_model.zip"
    if best_path.exists():
        named_best = model_dir / f"dqn_er_{name}_seed{args.seed}{expert_suffix}_best.zip"
        best_path.replace(named_best)
        print(f"Saved best ER model: {named_best.relative_to(ROOT)}")
    print(f"Saved final ER model: {final_path.relative_to(ROOT)}")

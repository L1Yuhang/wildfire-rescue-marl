...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate PPO variants and baselines.")
...
class PPOPolicy:
    def __init__(self, model: PPO, mode: str, env_config: dict, safe_projection: bool) -> None:
        self.model = model
        self.mode = mode
        self.env_config = env_config
        self.safe_projection = safe_projection
...
def main() -> None:
    ...
    ppo_policy: PPOPolicy | None = None
    if args.include_ppo:
        model_path = resolve_model_path(name, args.mode, 0, args.model_path, args.model_kind)
        ppo_policy = PPOPolicy(PPO.load(model_path), args.mode, env_config, safe_projection)
    ...
    if ppo_policy is not None:
        ppo_name = f"ppo_{args.mode}"
        for episode in range(args.episodes):
            metrics = ppo_policy.run_episode(args.seed + episode)
            metrics.update({"agent": ppo_name, "episode": episode, "episode_seed": args.seed + episode})
            rows.append(metrics)

    out_dir = ROOT / "outputs" / "eval" / "metrics_csv"
    ensure_dirs(out_dir)
    tag = f"_{args.output_tag}" if args.output_tag else ""
    detail_path = out_dir / f"ppo_all_{name}_{args.mode}_seed{args.seed}{tag}.csv"
    summary_path = out_dir / f"ppo_summary_{name}_{args.mode}_seed{args.seed}{tag}.csv"
